    ...
    switch cycleType
        case 'charge'
            ...
            for j = 1:length(cycleData.Time)
                ...
                %dataStructArray(j).Time = second(cycleData.Time(j));
                
                dataStructArray(j).Time = datetime(cycleData.Time(j),'ConvertFrom','posixtime','Format','HH:mm:ss.SSS');
            ...
            dataTimetable = table2timetable(dataTable);

            % Creazione di una nuova riga per il dataset
            newRow = {cycleType, cycleAmbient, datetime(cycleTime, 'Format', 'dd-MM-yyyy HH:mm:ss'), dataTimetable};
        ...
        case 'discharge'
            ...
            % Creazione di una nuova riga per il dataset
            newRow = {cycleType, cycleAmbient, datetime(cycleTime, 'Format', 'dd-MMM-yyyy HH:mm:ss.SSS'), dataTimeTable};
            dataset = [dataset; newRow];
        ...
        case 'impedance'
            ...
            % Creazione di una nuova riga per il dataset
            newRow = {cycleType, cycleAmbient, datetime(cycleTime, 'Format', 'yyyy-MM-dd HH:mm:ss'), dataTimeTable};
            dataset = [dataset; newRow];
        
            %}
   end
end
%% Conversione del dataset primario e salvataggio dei dati estratti in un file .mat

dataset = table2timetable(dataset); 

save('dataset.mat', "dataset");%% Caricamento del dataset 
load('B0005.mat'); 
